...
pub fn built_in_defaults() -> ImageTemplates {
    ...
    ImageTemplates {
        version: 1,
        rotation: vec!["main".into(), "hero".into(), "food_grid".into(), "scene".into(), "guarantee".into()],
        templates: vec![
            t("main", "main", "主图(纯白)", "model", true, format!(
            ...
            t("hero", "promo", "信息图 Hero", "model", true, format!(
            ...
            t("food_grid", "gallery", "结果九宫格", "model", true, format!(
                "Using THIS exact product, create a vertical 3:4 Wildberries infographic. A {{ACCENT}} top banner with a short white Russian headline «{{TITLE}}». Below it a clean 2x2 grid of appetizing result photos for a {{CATEGORY}}: {{SCENE}}. Place a small inset of this exact product in one corner. Vivid professional photography. {GUARD}"
            )),
            t("scene", "gallery", "生活场景", "model", true, format!(
                "Using THIS exact product (unchanged), create a vertical 3:4 Wildberries lifestyle infographic. Place it naturally in {{SCENE}}, with premium photography and warm natural light. Add a {{ACCENT}} banner with a short white Russian headline «{{TITLE}}». Place a small clean inset of this exact product in a top corner. {GUARD}"
            )),
            t("guarantee", "gallery", "质保/卖点徽章", "model", true, format!(
            ...
            // ── advanced (off by default; text-heavy → overlay recommended) ──
            t("dimensions", "gallery", "尺寸图(叠字)", "overlay", false, format!(
                "Keep THIS exact product unchanged. Clean white studio photo of the product, centered, leaving generous empty margins on the right and bottom for dimension labels. Soft shadow, NO text, NO numbers. {GUARD}"
            )),
            t("how_to", "gallery", "三步用法", "model", false, format!(
